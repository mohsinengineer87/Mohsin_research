...
# ===============================
# 🎯 Threat Vectors Subcategories (Affecting T in Effort Equation)
# ===============================
with st.expander('🎯 Threat Vectors'):
    threat_standard = st.radio('Select Threat Standard:', ['MITRE ATT&CK', 'ENISA Threat Landscape', 'VERIS'], key='key_radio_0')
    if threat_standard == 'MITRE ATT&CK':
        selected_vectors = st.multiselect('Select MITRE Techniques:', [ key='key_multiselect_1')
            'Phishing (T1566.001)',
    ...
    elif threat_standard == 'ENISA Threat Landscape':
        selected_vectors = st.multiselect('Select ENISA Threats:', [ key='key_multiselect_2')
            'Ransomware', 'Phishing', 'Data Breaches', 'Insider Threats', 'Denial of Service'
        ])
    elif threat_standard == 'VERIS':
        selected_vectors = st.multiselect('Select VERIS Threat Actions:', [ key='key_multiselect_3')
            'Malware', 'Hacking', 'Social', 'Misuse', 'Physical', 'Error'
        ])
    else:
        selected_vectors = []
    T = len(selected_vectors) if selected_vectors else 1
    st.write('Threat Vector Count (T):', T)
# ===============================
...
with st.sidebar:
    ...
    num_samples = st.slider("Number of Samples", 50, 20000, 500, help="How many rows to generate.", key='key_slider_4')
    scaling_factor = st.slider("Scaling Factor (S)", 0.1, 3.0, 1.1, 0.1, key='key_slider_5')
    rng_seed = st.number_input("Random Seed", value=42, help="For reproducibility.", key='key_num_6')

    st.header("🎛️ Attribute Ranges")

    # Assets: count + value
    asset_range = st.slider("Assets – Count (A)", 10, 20000, (100, 1000, key='key_slider_7')
    ...
    asset_value = st.slider(, key='key_slider_8')
        "Asset Value (1–100)",
        1, 100, 60,
    ...
    # Threat vectors
    threat_range = st.slider("Threat Vectors (T)", 1, 10000, (10, 200, key='key_slider_9')

    # Vulnerabilities + CVSS
    vuln_range = st.slider("Vulnerabilities – Count (V)", 1, 50000, (50, 1000, key='key_slider_10')
    ...
    cvss_baseline = st.slider(, key='key_slider_11')
    ...
    complexity_range = st.slider("Complexity (C)", 1, 10, (2, 7, key='key_slider_12')
    change_rate_range = st.slider("Change Rate (R)", 1, 10, (2, 7, key='key_slider_13')
    maturity_range = st.slider("Org Maturity (M)", 1, 10, (2, 7, key='key_slider_14')
    automation_range = st.slider("Automation (Au)", 1, 10, (2, 7, key='key_slider_15')

    st.header("🔊 Noise Settings")
    noise_type = st.selectbox("Noise Distribution", ["None", "Gaussian", "Laplace"])
    noise_pct = st.slider(, key='key_slider_16')
        "Noise Scale (% of mean Predicted)", 0.0, 100.0, 5.0, 0.5,
        help="Standard deviation (Gaussian) or b (Laplace) as % of mean(Predicted)."
    )
    noise_seed = st.number_input("Noise Seed", value=123, help="Separate seed for noise.", key='key_num_17')
    ...
    ofat_points = st.slider(, key='key_slider_18')
...
# ===============================
# 🎯 MITRE ATT&CK Threat Vector Selection
# ===============================
with st.expander('🎯 MITRE ATT&CK Threat Vectors'):
    selected_techniques = st.multiselect(, key='key_multiselect_19')
    ...
    st.write('Selected Techniques:', selected_techniques)

# Example inputs (these should be replaced with user inputs or model-derived values)
A_count = st.number_input('Asset Count', min_value=1, value=50, key='key_num_20')
A_value = st.slider('Asset Value (1-100)', 1, 100, 80, key='key_slider_21')
V_count = st.number_input('Vulnerability Count', min_value=1, value=30, key='key_num_22')
CVSS = st.slider('CVSS Score (0-10)', 0.0, 10.0, 7.5, key='key_slider_23')
C = st.slider('Complexity Factor', 0.5, 3.0, 1.2, key='key_slider_24')
R = st.slider('Change Rate', 0.5, 3.0, 1.3, key='key_slider_25')
M = st.slider('Organizational Maturity (1–10)', 1, 10, 5, key='key_slider_26')
Au = st.slider('Automation Level', 1, 10, 2, key='key_slider_27')
S = st.slider('Scaling Factor', 1.0, 2.0, 1.1, key='key_slider_28')

# Threat vector count from selected MITRE techniques

# Effective asset and vulnerability calculations
A_eff = A_count * A_value
V_eff = V_count * CVSS

# Effort estimation
effort = ((A_eff * weighted_threat_score * V_eff * C * R) ** S) / (M * Au)
st.metric('📈 Estimated Modeling Effort', round(effort, 2))

# ===============================
# 📈 Effort vs Threat Vector Count Chart
# ===============================
import matplotlib.pyplot as plt
import numpy as np

# Generate a range of T values to simulate increasing threat vectors
T_range = np.arange(1, 21)  # T from 1 to 20
effort_values = [((A_eff * t * V_eff * C * R) ** S) / (M * Au) for t in T_range]

fig, ax = plt.subplots()
ax.plot(T_range, effort_values, marker='o')
ax.set_xlabel('Number of Threat Vectors (T)')
ax.set_ylabel('Estimated Effort')
ax.set_title('Scalability: Effort vs. Threat Vector Count')
st.pyplot(fig)
...
st.download_button(
    label='📥 Download Effort Chart as PNG',
    data=chart_buf.getvalue(),
    file_name='effort_vs_threat_vectors.png',
    mime='image/png'
)
