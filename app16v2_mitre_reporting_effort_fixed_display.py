...
# ---------------- Enhanced MITRE Reporting ----------------
if selected_techniques:
    st.subheader("🧾 Selected MITRE Techniques and Weights")
    for t in selected_techniques:
        st.write(f"- {t}: Weight = {technique_weights.get(t)}")
    st.write(f"📦 Computed Threat Weight (T_weight): **{T_weight}**")

# ---------------- Effort Preview ----------------
try:
    base_effort = df["PredictedEffort"].iloc[0]
    st.metric("📊 Predicted Effort (Sample Row)", round(base_effort, 2))

    # Comparison against T_weight=1.0 baseline
    baseline_effort = model_predicted(df.copy(), scaling_factor, 1.0).iloc[0]
    st.write("### 🔄 Comparison")
    st.write(f"⚪ Baseline Effort (No Threats): **{round(baseline_effort, 2)}**")
    st.write(f"🟢 Effort With Selected Threats: **{round(base_effort, 2)}**")

    import matplotlib.pyplot as plt
    fig, ax = plt.subplots()
    ax.bar(["No Threats", "Selected MITRE"], [baseline_effort, base_effort], color=["gray", "green"])
    ax.set_ylabel("Effort")
    ax.set_title("Effort Comparison")
    st.pyplot(fig)

except Exception as e:
    st.warning(f"Could not compute preview effort: {e}")
...
def empirical_elasticity(var, base_row, rel_delta=0.05):
    ...
    E0 = float(model_predicted(pd.DataFrame([row0]), scaling_factor, T_weight).iloc[0])
    E1 = float(model_predicted(pd.DataFrame([row1]), scaling_factor, T_weight).iloc[0])
