...
st.session_state['T_weight'] = T_weight if T_weight > 0 else 1.0
...
with st.sidebar:
    ...
    complexity_range = st.slider("Complexity (C)", 1, 10, (2, 7), help="1=Monolith, 3=Microservices, 5=Distributed Cloud/AI System")
    change_rate_range = st.slider("Change Rate (R)", 1, 10, (2, 7), help="1=Annual Updates, 3=Monthly Agile Releases, 5=CI/CD Daily")
    maturity_range = st.slider("Org Maturity (M)", 1, 10, (2, 7))
    automation_range = st.slider("Automation (Au)", 1, 10, (2, 7), help="1=Manual Excel, 3=Semi-Automated Tools, 5=CI/CD Integrated")
    ...
    st.markdown("---")
    with st.expander("📘 Industry Benchmarks for C, R, D"):
        st.markdown("### Complexity (C)")
        st.markdown("""
| Level | Description                             | Example Systems                        |
|-------|-----------------------------------------|----------------------------------------|
| 1     | Monolith, flat architecture             | Static sites, single VM                |
| 3     | Microservices, API-based apps           | CRM, ERP, e-commerce                   |
| 5     | Distributed cloud-native w/ AI or IoT   | 5G Core, Smart Cities, ML Pipelines    |
        """)

        st.markdown("### Change Rate (R)")
        st.markdown("""
| Level | Update Frequency        | Examples                                |
|-------|-------------------------|-----------------------------------------|
| 1     | Yearly / Infrequent     | Legacy on-premise systems               |
| 3     | Monthly Agile releases  | Internal tools, regular web apps        |
| 5     | CI/CD or daily deploys  | Fintech APIs, ML pipelines, SaaS        |
        """)

        st.markdown("### Automation (D)")
        st.markdown("""
| Level | Description                         | Tooling Examples                          |
|-------|-------------------------------------|-------------------------------------------|
| 1     | Manual (Word, Excel)                | None                                      |
| 3     | Semi-automated                      | OWASP ThreatDragon, STRIDE templates       |
| 5     | CI/CD integrated & fully automated  | IriusRisk, ThreatModeler, APIs            |
        """)
...
def model_predicted(df, S, T_weight=1.0):
    """
    Effort model using:
...
ofat_df["PredictedEffort"] = model_predicted(ofat_df, scaling_factor, T_weight=st.session_state["T_weight"])
