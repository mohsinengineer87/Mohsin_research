...
# ---------------- Refresh Predicted Effort Dynamically ----------------
df = df.copy()  # avoid modifying original

df["PredictedEffort_WithThreats"] = model_predicted(df, scaling_factor, T_weight)
df["PredictedEffort_NoThreats"] = model_predicted(df, scaling_factor, 1.0)
df["EffortDelta"] = df["PredictedEffort_WithThreats"] - df["PredictedEffort_NoThreats"]

st.subheader("📉 Effort Table With and Without MITRE Threats")
st.dataframe(df[["PredictedEffort_NoThreats", "PredictedEffort_WithThreats", "EffortDelta"]])

df["ActualEffort"] = add_noise(df["PredictedEffort_WithThreats"], noise_type, noise_pct, seed=noise_seed)
