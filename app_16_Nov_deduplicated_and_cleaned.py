...
with st.expander('🎯 Threat Vectors'):
    threat_standard = st.radio('Select Threat Standard:', ['MITRE ATT&CK', 'ENISA Threat Landscape', 'VERIS'])
...
A_count = st.number_input('Asset Count', min_value=1, value=50)
A_value = st.slider('Asset Value (1-100)', 1, 100, 80)
V_count = st.number_input('Vulnerability Count', min_value=1, value=30)
CVSS = st.slider('CVSS Score (0-10)', 0.0, 10.0, 7.5)
C = st.slider('Complexity Factor', 0.5, 3.0, 1.2)
R = st.slider('Change Rate', 0.5, 3.0, 1.3)
M = st.slider('Organizational Maturity (1–10)', 1, 10, 5)
Au = st.slider('Automation Level', 1, 10, 2)
S = st.slider('Scaling Factor', 1.0, 2.0, 1.1)
...
# Effort estimation
effort = ((A_eff * T * V_eff * C * R) ** S) / (M * Au)
